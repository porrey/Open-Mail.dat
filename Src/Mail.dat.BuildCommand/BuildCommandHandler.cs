	...
	internal class BuildCommandHandler : ModelCommand<CommandOptions>
	{
		...
		protected override async Task<int> OnHandleCommand(CommandOptions options)
		{
			...
			ClassBuilder.Create(ContextName)
				...
				.AddMethod(MethodBuilder.Create("Dispose")
					.SetScope("public override")
					.SetReturnType("void")
					.AddCode("this.Database.CloseConnection();")
					.AddCode("this.Logger?.LogDebug(\"Disposed database context {context}.\", nameof(MaildatContext));")
					.AddCode("base.Dispose();")
				)
				.AddMethod(MethodBuilder.Create("DisposeAsync")
					.SetScope("public override")
					.SetReturnType("ValueTask")
					.AddCode("this.Database.CloseConnection();")
					.AddCode("this.Logger?.LogDebug(\"Disposed database context {context}.\", nameof(MaildatContext));")
					.AddCode("return base.DisposeAsync();")
				)
